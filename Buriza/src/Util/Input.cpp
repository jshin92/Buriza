...
bool Input::m_discretelyPressedKeys[MAX_INPUT_SIZE]{};

void Input::ResetDiscreteKeyPresses()
{
    std::fill(m_discretelyPressedKeys, m_discretelyPressedKeys + MAX_INPUT_SIZE, false);
}
...
void Input::KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    {
        if (m_keys[key]) m_discretelyPressedKeys[key] = true;
        m_keys[key] = false;
    }
}
...
{
    return m_discretelyPressedKeys;
}
